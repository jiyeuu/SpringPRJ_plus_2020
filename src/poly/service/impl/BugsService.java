...
public class BugsService implements IBugsService {
	...
	public int collectBugsRank() throws Exception {
		...
		while (it.hasNext()) {
			...
			nDTO.setWord(str);
			sDTO = nlpService.preProcessWordAnalysisForMind2(nDTO);
			
			pDTO.setHap2(Integer.parseInt(sDTO.getHap()));
			pDTO.setPos2(Integer.parseInt(sDTO.getPos()));
			pDTO.setNes2(Integer.parseInt(sDTO.getNes()));
	...
	@Override
	public List<SongDTO> getRank1() throws Exception {

		log.info(this.getClass().getName() + ".getRank1 Start!");

		// 조회할 컬렉션 이름
		String colNm = "BugsTOP100_" + DateUtil.getDateTime("yyyyMMdd"); 

		List<SongDTO> sList = bugsMapper.getRank1(colNm);

		if (sList == null) {
			sList = new ArrayList<SongDTO>();	
		}
		
		log.info(this.getClass().getName() + ".getRank1 End!");
